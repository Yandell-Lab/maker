#-------------------------------------------------------------------------------
#------                     CGL::Annotation::GFF3::Maker                 -------
#-------------------------------------------------------------------------------
package CGL::Annotation::GFF3::Maker_snap;
use strict;
...
sub new {
	...
	my ($genes, $seq, $seq_id) = get_genes($file,'exon', 0);
...
sub to_gff3_gene {
	...
	my @fields;
        ...
        push(@fields, $seg_id, 'snap', 'gene', $g->{i_start});
...
sub to_gff3_mRNA {
	...
	my $t_n    = $t->{f}->get_Annotations('Name')->value();
        ...
        push(@fields, $seg_id, 'snap', 'mRNA', $t->{i_start});
...
sub to_gff3_exon {
        ...
        my $t_id   = $t->{f}->get_Annotations('ID')->value();
        my $e_id   = $e->{f}->get_Annotations('ID')->value();

        my $strand = $e->{f}->strand() == 1 ? '+' : '-';

        my @fields;

        push(@fields, $seg_id, 'snap', 'exon', $e->{i_start});
...
sub to_gff3_cds {
        ...
        my $t_id   = $t->{f}->get_Annotations('ID')->value();
        my $c_id   = $c->{f}->get_Annotations('ID')->value();

        my $strand = $c->{f}->strand() == 1 ? '+' : '-';

        my @fields;

        push(@fields, $seg_id, 'snap', 'exon', $c->{i_start});
...
sub load_exon {
	...
	$exon->{feature_id} = $e->{f}->get_Annotations('ID')->value();
	$exon->{id}         = $e->{f}->get_Annotations('ID')->value();
	$exon->{inScope}    = 1;
	...
	$exon->{name} = $e->{f}->get_Annotations('ID')->value();


	my $sF_id = $e->{f}->get_Annotations('ID')->value();
	...
	$exon->{uniquename} = 
	$e->{src_f_id}.':'.$e->{f}->get_Annotations('ID')->value();
...
sub get_genes {
	...
	my $exons = grab($base_type, 'snap', $features, $c_id);
	my $cdss  = grab('exon', 'snap', $features, $c_id);
	my $mRNAs = grab('mRNA', 'snap', $features, $c_id);
...
sub sort_exons {
	...
	if    ($t->{f}->strand() ==  1){
		...
		die "unknown strand in GFF3::Maker_snap::sort_exons!\n";
	}
	return \@sorted;
}
#-------------------------------------------------------------------------------
