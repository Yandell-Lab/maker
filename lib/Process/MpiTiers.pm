...
#--------------------------------------------------------------
#runs the MpiTier until multiple chunks are available to distribute
...
#calls $self->run to try and advance the MpiTier
...
sub _load_chunks_for_level {
   ...
   #--select variables to send to Process::MpiChunk object
   if ($level == 0) {
      #----------------------CLEAR_MEMORY
...
sub _build_chunk {
   ...
   my $chunk = Process::MpiChunk->new($level, $args, $chunk_id,  $self->{LOG});
...
#--------------------------------------------------------------
#returns a copy of this MpiTiers object
