...
sub new {
   ...
   if (@args) {
      my $arg = shift @args;
      if (ref $arg eq 'Process::IPRtiers') {
	 $self = $arg->clone();
      }
      else {
...
#--------------------------------------------------------------
#initializes variables in the IPRtier object
...
sub _initialize_level {
   ...
   if (! defined $level){ #level will be undefined when finished
      $self->{TERMINATE} = 1;
      $self->{DS} = "$self->{VARS}{seq_id}\tFINISHED";
...
sub _next_level {
   ...
   try {
      $next_level = Process::IPRchunk::flow($level, $self->{VARS});
   }
   catch Error::Simple with {
      my $E = shift;
...
sub _load_chunks {
   ...
   try {
      $chunks = Process::IPRchunk::loader($level, $self->{VARS}, $self->id);
   }
   catch Error::Simple with {
      my $E = shift;
...
sub update_chunk {
   ...
   if($tier_id != $self->id()){
      die "ERROR:  This IPRchunk is not part of this IPRtier\n";
   }
   if($level_num != $self->{LEVEL}{CURRENT}){
      die "ERROR:  This IPRchunk is not part of this level\n";
   }
...
#--------------------------------------------------------------
#returns a copy of this IPRtiers object using the Storable
