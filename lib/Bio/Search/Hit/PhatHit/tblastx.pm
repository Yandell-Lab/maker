...
=head1 Bio::Search::Hit::PhatHit::tblastx

Bio::Search::Hit::PhatHit::tblastx - The platonic ideal of a CGL module.

=head1 SYNOPSIS

=for example
 use Bio::Search::Hit::PhatHit::Base;
 my $hits = Bio::Search::Hit::PhatHit::Base::_getTestHits('tblastx',
...
=for example_testing
  isa_ok($hit, "Bio::Search::Hit::PhatHit::tblastx", "check type.");
...
package Bio::Search::Hit::PhatHit::tblastx;
...
=for example_testing
  ...
  is($h_natural_begin, 763, "Check the hit's natural begin.");
...
sub nB
 {
	...
	if    ($self->strand($w) == -1){
		...
		die "dead in tblastx::PhatHit::nB\n";
	}
}
...
=for example_testing
  # XXXX check off by one w/ Mark.
  is($q_natural_end, 4968, "Check the query's natural end.");
  # XXXX this is dead on....
  is($h_natural_end, 93078, "Check the hit's natural end.");
	...
	my $hsp;
		...
		$hsp = pop(@{$sorted});
               ...
               return  $hsp->end($w);
                ...
                die "dead in tblastx::PhatHit::nE\n";
        }
...
sub AUTOLOAD
 {
        ...
        if($ENV{CGL_CHATTER}) {
	    print STDERR "tblastx::PhatHit::AutoLoader called for: ",
	    "\$self->$call","()\n";
