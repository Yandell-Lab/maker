...
=head1 Bio::Search::Hit::PhatHit::blastx

Bio::Search::Hit::PhatHit::blastx - The platonic ideal of a CGL module.
...
package Bio::Search::Hit::PhatHit::blastx;
...
sub nB
 {
	...
	if ($self->strand($w) eq '-1/1'){
        ...
        elsif ($self->strand($w) == 0){
                $sorted = $self->sortFeatures($w);
                $hsp = shift(@{$sorted});
                return  $hsp->start($w);
        }
        else {
                die "dead in blastx::PhatHit::nB\n";
        }
}
...
sub AUTOLOAD
 {
        ...
        if($ENV{CGL_CHATTER}) {
	    print STDERR "blastx::PhatHit::AutoLoader called for: ",
	    "\$self->$call","()\n";
